...
DETECTION_THRESHOLD = 0.01 # Confidence threshold for YOLO detections (adjust as needed) [cite: 5]

# --- Density Analysis Settings (remain the same) --- [cite: 5]
HIGH_DENSITY_THRESHOLD = 3
CRITICAL_DENSITY_THRESHOLD = 6
HIGH_DENSITY_CELL_COUNT_THRESHOLD = 2
CRITICAL_DENSITY_CELL_COUNT_THRESHOLD = 1
GRID_ROWS = 10
GRID_COLS = 10
STATUS_HIERARCHY = {
...
def upload_media_route():
    ...
    if file_type == 'video':
        ...
        try:
            ...
            end_time = time.time()  # End FPS timer
            processing_time = end_time - start_time

            if frame_num > 0 and processing_time > 0:
                avg_fps = frame_num / processing_time
                print(f"Average Processing FPS: {avg_fps:.2f}")
            else:
                avg_fps = 0
                print("FPS could not be calculated (no frames or zero time).")    # if frame_num % 50 == 0: print(f"  Processed frame {frame_num}...")
...
def generate_live_frames(camera_index=0): # Default to 0 [cite: 84]
    ...
    try:
        ...
        if not live_cap.isOpened():
            ...
            fallback_video_path = "vid 22 22s.mp4" # [cite: 88]
